...
public class RouteServiceImpl implements RouteService {
    ...
    public void delete(Long id) {
        var route = routeRepository.findById(id)
                .orElseThrow(() -> new NotFoundException("Route %d not found or was deleted yet".formatted(id)));
        routeRepository.delete(route);
    }

    @Override
    public RouteResponse getByCode(String code) {
        return routeRepository.findByCode(code).map(mapper::toResponse)
                .orElseThrow(() -> new NotFoundException("Route with code %s not found".formatted(code)));
    }

    @Override
    public RouteResponse getByName(String name) {
        return routeRepository.findByName(name).map(mapper::toResponse)
                .orElseThrow(() -> new NotFoundException("Route with name %s not found".formatted(name)));
    }
